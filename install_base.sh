...
function config_vscode() {
    ln -nsf $ROOT/vscode ~/.config/Code/User
    echo "You may need install font manully\n"
}

function config_sources() {
    # change source of apt-get
    if [ ! -e '/etc/apt/sources.list.bak' ]; then
        sudo mv /etc/apt/sources.list /etc/apt/sources.list.bak
    fi
    rm ./sources.list
    sudo echo "deb http://mirrors.163.com/debian/ jessie main non-free contrib
